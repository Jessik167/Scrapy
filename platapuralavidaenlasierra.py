...
class platapuralavidaenlasierra(scrapy.Spider):
    name = 'platapuralavidaenlasierra'
    _num_pagina = 1
    start_urls = ['platapuralavidaenlasierraalbummp3.wordpress.com']
    ...
    def parse(self, response):
        referer = response.css('h2 > a ::attr(href)').get()
        Request(referer)
        #####COMENTARIOS#####
        '''print('\n########Pagina ' + str(self._num_pagina) + '########')
        #####TOMA LOS DATOS DE LA P�GINA#####
